...
function setup() {
  ...
  ground = createSprite(200,windowHeight-215,400,20);
  ground.addImage("ground",groundImage);
  ground.x = ground.width /2;
  ground.velocityX = -(6 + 3*score/100);
  
  gameOver = createSprite(windowWidth/2,windowHeight-370);
  
  gameOver.addImage(gameOverImg);
  
  restart = createSprite(windowWidth/2,windowHeight-320);
  ...
  invisibleGround = createSprite(200,windowHeight-200,400,10);
...
function draw() {
  ...
  console.log(windowHeight-218);
  
  if (gameState===PLAY){
    score = score + Math.round(getFrameRate()/60);
    ground.velocityX = -(6 + 3*score/100);
  
    if(touches.lengh>0||keyDown("space") && trex.y >= windowHeight-248) {
      trex.velocityY = -12;
    }
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(600,windowHeight-226,10,40);
    //obstacle.debug = true;
